...
def check_livestock_run_data(mgmt_dir, anml_prodn_obj):
    """
    Test livestock data
    animal production object includes crop names for validation purposes
    """

    # read and validate livestock JSON files
    # ======================================
    all_lvstck = ReadLivestockSheet(mgmt_dir, anml_prodn_obj)
    _get_production_and_n_excreted(anml_prodn_obj, all_lvstck)    # updates

    if len(all_lvstck.subareas) > 0:
        print('subareas to process: ' + ' '.join(list(all_lvstck.subareas.keys())))
        return True
    else:
        print(WARN_STR + 'no subareas for livestock to process in ' + join(mgmt_dir, FNAME_RUN))
        return False
...
def _get_production_and_n_excreted(anml_prodn_obj, all_lvstck):
    ...
    """
    anml_prodn_df = anml_prodn_obj.anml_prodn_df
...
def calc_livestock_data(form):
    ...
    # =================================================================================
    anml_prodn_obj = form.anml_prodn

    # read and validate livestock JSON files
    # ======================================
    all_lvstck = ReadLivestockSheet(form.settings['mgmt_dir'], anml_prodn_obj)
    _get_production_and_n_excreted(anml_prodn_obj, all_lvstck)    # updates
